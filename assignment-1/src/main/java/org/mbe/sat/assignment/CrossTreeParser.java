...
public class CrossTreeParser {
	...
	private void changeNumberToNames() {
		cnfCrossTree = cnfCrossTree.replace("(", "");
		cnfCrossTree = cnfCrossTree.replace(")", "");
		cnfCrossTree = cnfCrossTree.replace(" ", "");
		
		Scanner and = new Scanner(cnfCrossTree);
		and.useDelimiter("&");
		
		String subCNF = "";
		
		while(and.hasNext()) {	    	
			Scanner or = new Scanner(and.next());
		    or.useDelimiter("\\|"); 
		    ...
		    while(or.hasNext()) {
		    	...
		    	for(int i=0; i<featureList.size(); i++) {
			    	if(next.equals(String.valueOf(featureList.get(i).getNumber()))) {
			    		if(not)
			    			subCNF += "~" + featureList.get(i).getName();
			    		else
			    			subCNF += featureList.get(i).getName();
			    	}	    		
			    }
		...
		and.close();
		subCNF = subCNF.substring(0, subCNF.length() - 2);
		cnfCrossTree = subCNF;
	}
