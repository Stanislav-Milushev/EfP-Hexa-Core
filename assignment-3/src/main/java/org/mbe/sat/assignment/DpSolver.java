...
public class DpSolver implements ISolver<CnfFormula, Optional<Assignment>> {
    ...
    public Optional<Assignment> solve(CnfFormula cnfFormula) {
        ...
        return DPAlgorithmus(cnfFormula, assignment);
        
    }
    
    /**
     * DP-Algorithm.
     *	Recursive iteration through the decision-tree until a suitable solution was found.
     *	Cancels decision paths that cant satisfy the formula.
     * @param cnfFormula the formula that is solved
    ...
    /**
     * Fills unused variables (not assigned) to an assignment that satisfies the formula.
    ...
    /**
     * Checks if a variable is assigned to given assignment. Function is used by getNextUnusedVariable(...).
    ...
    private boolean hasEmptyClause(CnfFormula cnfFormula) {
    	 Set<Or<Atom>> clausesWithBottom = cnfFormula.getOperands().stream()
                 ...
                 .filter(this::containsEmptyClause)
